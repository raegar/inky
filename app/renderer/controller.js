...
InkProject.setEvents({
    "newProject": (project) => {
        setInkFileWhenReady(() => project.mainInkFile);
        EditorView.focus();
...
ipc.on("set-audio-controls-visible", (event, visible) => {
    PlayerView.setAudioControlsVisible(visible);
});
...
function setInkFileWhenReady(getInkFile, callback) {
    const trySet = () => {
        ...
        if (inkFile) {
            PlayerView.setCurrentInkFile(inkFile);
