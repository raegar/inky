...
function ProjectWindow(filePath) {
    ...
    this.browserWindow.webContents.on('dom-ready', () => {
        ...
        // Default to showing controls when undefined
        this.browserWindow.webContents.send('set-audio-controls-visible', settings.audioControlsVisible === undefined ? true : !!settings.audioControlsVisible);
        // Default to unmuted when undefined
        this.browserWindow.webContents.send('set-audio-muted', settings.audioMuted === undefined ? false : !!settings.audioMuted);
    });
...
ProjectWindow.getViewSettings = function() {
    let viewSettingDefaults = { theme:'light', zoom:'100', animationEnabled:true, audioControlsVisible:true, audioMuted:false };
